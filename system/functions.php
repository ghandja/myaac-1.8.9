...
function getItemImage($id, $count = 1)
{
	...
	$tooltip = !empty($name) ? ' class="item_image" title="' . $name . '"' : '';

	$item_url = setting('core.item_images_url');
	$ext = setting('core.item_images_extension');
	$file_name = $id;
	if($count > 1)
		$file_name .= '-' . $count;

	$src = $item_url . $file_name . $ext;
	return '<img src="' . $src . '"' . $tooltip . ' width="32" height="32" border="0" alt="' .$id . '" onerror="this.style.display=\'none\';this.nextElementSibling.style.display=\'inline\';" /><span class="item-name-badge" style="display:none" title="ID: ' . $id . '">' . (!empty($name) ? $name : '#' . $id) . '</span>';
}
